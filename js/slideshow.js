(function ($) {
    ...
    var $backgrounds = $("<ul class='sm-slider'></ul>"),
        methods = {
            ...
            init: function (settings) {
                ...
                $('<style type="text/css" id="sm-slider-css"></style>').html(css).appendTo('head');
                $backgrounds.html(this.mkhtml(options.src)).prependTo("body");
            },

            // optionsを指定したものにガラッと変える this method add by eiichi-worker
            changeOptions: function (settings) {
                $.extend(options, settings);
                this.options = options;

                var css = this.mkcss(options.src.length, options.duration, options.fade);
                $('#sm-slider-css').html(css)
                $('.sm-slider').html(this.mkhtml(options.src))
            },
    ...
    // optionsを指定したものにガラッと変える this method add by eiichi-worker
    $.sublime_slideshow.changeOptions = function (options) {
        methods.changeOptions(options);
    };
